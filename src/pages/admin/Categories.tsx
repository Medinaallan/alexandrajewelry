...
const BLANK: CategoryForm = {
  name: '', nameEs: '', slug: '', image: '', description: '', descriptionEs: '', active: true,
};
...
export default function AdminCategoriesPage() {
  ...
  const openEdit = (cat: Category) => { setEditing(cat); setForm({ ...cat }); setModalOpen(true); };
      ...
      <div className="admin-content p-8">
              ...
              <div className="p-4 flex flex-col gap-2">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{cat.nameEs}</p>
                  </div>
                  <button
                    onClick={() => updateCategory(cat.id, { active: !cat.active })}
                    className="hover:text-[--gold] transition-colors text-[--text-muted] flex-shrink-0"
                  >
                    {cat.active
                ...
                <div className="flex items-center gap-2 pt-2" style={{ borderTop: '1px solid var(--border)' }}>
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 text-xs text-[--text-muted] hover:text-red-500 transition-colors ml-auto"
                  >
        ...
        <Modal isOpen={!!confirmDelete} onClose={() => setConfirmDelete(null)} title={t('common.delete')} size="sm">
          ...
          <div className="flex gap-3">
            <Button variant="danger" onClick={() => { deleteCategory(confirmDelete!); setConfirmDelete(null); }}>
              {t('common.delete')}
            </Button>
